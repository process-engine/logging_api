...
```
meta exec "any command" --exclude logging_api_core,logging_api_contracts
meta exec "any command" --include-only logging_api_core,logging_api_contracts
```
...
```
meta exec "git flow feature start my_feature" --include-only logging_api,logging_api_contracts
```

* Starts the feature "my_feature" in the modules
`logging_api` and `logging_api_contracts`

### 3.7 Publish a feature on multiple repositories

```
meta exec "git flow feature publish my_feature" --include-only logging_api,logging_api_contracts
```
