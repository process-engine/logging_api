...
async function getVersionFromNpmTag(npmTag) {
  ...
  const tagsAsString = await exec('npm dist-tag ls @process-engine/logging_api_contracts');
...
async function setContractsVersionInLayerPackages(versionToUse) {
  ...
  console.log(`Setting @process-engine/logging_api_contracts to ${versionToUse} for all layer packages`);

  setPersistenceApiContractsBranchVersionForPackage('logging.repository.file_system', versionToUse);
  setPersistenceApiContractsBranchVersionForPackage('logging.service', versionToUse);
}

function setPersistenceApiContractsBranchVersionForPackage(packageName, versionToSet) {

  console.log(`Setting @process-engine/logging_api_contracts for package ${packageName}`)
  ...
  packageJson.dependencies['@process-engine/logging_api_contracts'] = versionToSet;
...
async function createCommit() {
  ...
  try {
    ...
    await exec('git commit -m ":recycle: Update versions for logging_api_contracts"');
